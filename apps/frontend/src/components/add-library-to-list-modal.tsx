...
export function AddLibraryToListModal({
  ...
  const [sortBy, setSortBy] = useState<MediaSortBy>(MediaSortBy.TITLE);
  ...
  const visibleItems = useMemo(() => {
    ...
    return availableItems
      ...
      .sort((a, b) => compareMediaItems(a, b, sortBy));
  }, [availableItems, search, typeFilter, sortBy]);

  const sortLabel = t(
    MEDIA_SORT_OPTIONS.find((option) => option.value === sortBy)?.labelKey ??
      'filters.sort',
  );
      ...
      >
        <div className="space-y-4">
          <label className="block space-y-1.5">
            ...
            <select
              value={sortBy}
              onChange={(event) => setSortBy(event.target.value as MediaSortBy)}
              className={fieldClass}
              data-autofocus
            >
